...
class MarketingRequest(models.Model):
    ...
    storepromotion = fields.Float(
        string='Store Promotion AMT(INR)', 
        compute='_compute_promotion_amounts',
        store=True,
        readonly=True
    )
    productpromotion = fields.Float(
        string='Product Promotion AMT(INR)', 
        compute='_compute_promotion_amounts',
        store=True,
        readonly=True
    )
    influencerpromotion = fields.Float(
        string='Influencer Promotion AMT(INR)', 
        compute='_compute_promotion_amounts',
        store=True,
     readonly=True
    )

# Add this computed method to calculate promotion amounts
    @api.depends('budget_line_amount_ids.utilized_amount', 'budget_line_amount_ids.allocation_type')
    def _compute_promotion_amounts(self):
        """Calculate promotion amounts based on utilized amounts from budget lines"""
        for record in self:
            store_amount = 0.0
            product_amount = 0.0
            influencer_amount = 0.0
        
            for budget_line_amount in record.budget_line_amount_ids:
                if budget_line_amount.allocation_type == 'store':
                    store_amount += budget_line_amount.utilized_amount
                elif budget_line_amount.allocation_type == 'product':
                    product_amount += budget_line_amount.utilized_amount
                elif budget_line_amount.allocation_type == 'influencer':
                    influencer_amount += budget_line_amount.utilized_amount
        
            record.storepromotion = store_amount
            record.productpromotion = product_amount
            record.influencerpromotion = influencer_amount
    ...
    total_promotion = fields.Float(
        string='Total Promotion AMT(INR)',
        compute='_compute_total_promotion',
        store=True,
        readonly=True
    )

    @api.depends('storepromotion', 'productpromotion', 'influencerpromotion')
    def _compute_total_promotion(self):
        """Calculate total promotion amount"""
        for record in self:
            record.total_promotion = record.storepromotion + record.productpromotion + record.influencerpromotion
    completion_status = fields.Selection([
        ('not_started', 'Not Started'),
        ('in_progress', 'In Progress'),
        ('completed', 'Completed'),
        ('cancelled', 'Cancelled')  # Note: 'cancelled' not 'canceled'
    ...
    # Add these methods to your MarketingRequest class
    ...
    def action_cancel_completion(self):
        """Mark request as cancelled"""
        for record in self:
            record.completion_status = 'cancelled'

    def action_reset_not_started(self):
...
class MarketingRequestBudgetLineAmount(models.Model):
    ...
    utilized_amount = fields.Float(
        ...
        default=0.0,
        help="Amount actually utilized from this budget line"
    )
    
    # Add these missing fields that your view is trying to access:
    allocation_type = fields.Selection([
        ('store', 'Store Promotion'),
        ('product', 'Product Promotion'),
        ('influencer', 'Influencer Promotion')
    ...
    # This is the 'amount' field that was missing
    ...
    @api.depends('allocated_amount', 'unit_budget_line_id.utilized_amount')
    def _compute_available_amount(self):
        for record in self:
            if record.unit_budget_line_id:
                record.available_amount = record.allocated_amount - record.unit_budget_line_id.utilized_amount
            else:
                record.available_amount = 0.0
